...
extern unsigned char clock_reg[3], speed[3];
extern unsigned int i;
extern unsigned int block, clear_flag;
extern unsigned char dash_mode;
extern unsigned char *event[];
extern unsigned char prekey;
extern char pass[5];
...
void update_dashboard_display() {
    ...
    clcd_print(event[i], (0xC0 + 10));
    unsigned short adc_val = read_adc(0x00) / 10.23;
...
void handle_menu_navigation(unsigned char *down_press, unsigned int *long_press_counter) {
    ...
    if (*down_press == 0) {
        clcd_print("View Logs    ", (0x80 + 3));
        clcd_print("Clear Logs   ", (0xC0 + 3));
    } else if (*down_press == 2) {
        clcd_print("Clear Logs   ", (0x80 + 3));
        clcd_print("Download log ", (0xC0 + 3));
    } else if (*down_press == 4) {
        clcd_print("Download log ", (0x80 + 3));
        clcd_print("Rest Password", (0xC0 + 3));
    } else if (*down_press == 6) {
        clcd_print("Rest Password", (0x80 + 3));
        clcd_print("Reset Time   ", (0xC0 + 3));
    }
...
void display_logs() {
    ...
    while (1) {
        if (clear_flag == 0) {
            ...
            for (int j = 0; j < 8; j++) {
                clcd_putch(eeprom_data[j], (0xC0 + 2 + j));
            }

            for (int j = 0; j < 2; j++) {
                clcd_putch(eeprom_data[8 + j], (0xC0 + 11 + j));
            }

            for (int j = 0; j < 2; j++) {
                clcd_putch(eeprom_data[10 + j], (0xC0 + 14 + j));
            }
...
void change_pass(void) {
    ...
    while (pos < 4) {
        ...
        if (key == 0x2F || key == 0x1F) {
            new_pass[pos] = (key == 0x2F) ? '1' : '0';
            clcd_putch('*', (0xC0 + i));
            i++;
            pos++;
        }
    ...
    if (strcmp(new_pass, re_pass) == 0) {
        ...
        for (int addr = 125; addr < 129; addr++) {
            write_ext_eeprom(addr, new_pass[addr - 125]);
        }
        clcd_write(0x01, 0);
        clcd_print("Password is", (0x80 + 2));
        clcd_print("Changed", (0xC0 + 4));
    } else {
        clcd_write(0x01, 0);
        clcd_print("Password", (0x80 + 3));
        clcd_print("Mismatch", (0xC0 + 3));
    }
    _delay((unsigned long)((1000)*(20000000/4000.0)));
    clcd_write(0x01, 0);
    dash_mode = 1;
...
void download_log() {
    ...
    while (log_index < log_counter) {
        ...
        strncpy(time, eeprom_data, 8);
        time[8] = '\0';
        strncpy(gear, eeprom_data + 8, 2);
        gear[2] = '\0';
        strncpy(speed, eeprom_data + 10, 2);
        speed[2] = '\0';
...
void change_time(void) {
    ...
    while (1) {
        ...
        if (one_time) {
            ...
            new_time[0] = ((clock_reg[0] >> 4) & 0x03) * 10 + (clock_reg[0] & 0x0F);
            new_time[1] = ((clock_reg[1] >> 4) & 0x07) * 10 + (clock_reg[1] & 0x0F);
            new_time[2] = ((clock_reg[2] >> 4) & 0x07) * 10 + (clock_reg[2] & 0x0F);
        }


        if (key == 0x2F) {
            new_time[blink_pos]++;
            if (blink_pos == 0 && new_time[blink_pos] > 23)
                new_time[blink_pos] = 0;
            else if ((blink_pos == 1 || blink_pos == 2) && new_time[blink_pos] > 59)
                new_time[blink_pos] = 0;
        } else if (key == 0x1F) {
...
void enter_menu_option(unsigned char down_press) {
    ...
    } else if (down_press == 1 || down_press == 2) {
    ...
    else if (down_press == 3 || down_press == 4) {
        clcd_write(0x01, 0);
        download_log();
    ...
    } else if (down_press == 5 || down_press == 6) {
        change_pass();
...
int enter_password(char *pass, unsigned int *attempt) {
    ...
    if (strcmp(usr_pass, pass) == 0) {
        ...
        if (*attempt > 0) {
            ...
            clcd_putch(*attempt + '0', (0x80 + 0));
            clcd_print(" attempts left", (0x80 + 1));
            _delay((unsigned long)((1000)*(20000000/4000.0)));
        }
        return 0;
    }
